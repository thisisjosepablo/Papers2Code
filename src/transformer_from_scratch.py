...
class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query, mask):
        ...
        out = self.dropout(self.norm2(self.feed_forward(x) + x))
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            output = layer.forward(input, input, input, mask)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x,value, key, src_mask, trg_mask):
        ...
        out = self.transformer_block.forward(value, key, query, src_mask)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, enc_out, src_mask, trg_mask):
        ...
        for layer in self.layers:
            input = layer.forward(input, enc_out, enc_out, src_mask, trg_mask )
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(
            ...
            heads, forward_expansion, droput,
            device, max_length
        )
    ...
    def forward(self, src, trg):
        ...
        out = self.decoder.forward(trg,enc_out,src_mask, trg_mask)
...
if __name__ == "__main__":
    ...
    out = model.forward(x,trg[:,:-1]) # Target without the end because we want to predict that
    print(out.shape)

    # We get what is the predicted output by our transformer
    # In the real case, this predicted output is compared with the ground truth
    # and the weights are adjusted by using cross entropy or any loss function
